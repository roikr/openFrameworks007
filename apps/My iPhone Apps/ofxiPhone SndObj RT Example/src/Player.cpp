...
void Player::setup(ofxXmlSettings * xml,int soundSet,Mixer *main_mixer) {
	...
	for (int i=0; i<8;i++) {
		...
		smpl.inputf = new SndWave((char*)soundname.c_str(), READ);
		//smpl.buffer = new SndBuffer(1,smpl.inputf->GetDataFrames()*2,
		cout << smpl.inputf->GetDataFrames() << endl;
		smpl.inputs = new SndIn(smpl.inputf,1);
		smpl.inputs->Disable();
		smpl.gain = new Gain(0.,smpl.inputs);
