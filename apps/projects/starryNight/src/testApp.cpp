...
void testApp::setup(){
    ...
    device = xml.getValue("device", 0);
    //if you want to set a different device id 
	soundStream.setDeviceID(device); //bear in mind the device id corresponds to all audio devices, including  input-only and output-only devices.
	
    
    scale = xml.getValue("scale", 1.5);
    velocity = xml.getValue("velocity", -0.05);
    rot = ofVec2f(0.0,0.0)*xml.getValue("rotation", 0.0001) ;
    rest = xml.getValue("rest", 50);
    pondScale = xml.getValue("pondScale", 500);
...
void testApp::update(){
    ...
    for (vector<channel>::iterator iter=channels.begin(); iter!=channels.end(); iter++) {
        if (iter->bTriggered) {
            ...
            buffers[counter % buffers.size()][iy*(int)image.getWidth()+ix]= pondScale*y;
        }
    }
    ...
    for (vector<star>::iterator iter=stars.begin(); iter!=stars.end(); iter++) {
        ...
        if (iter->bTriggered && ofGetElapsedTimeMillis()-iter->triggerTime>3000) {
            iter->bTriggered = false;
        }
...
void testApp::drawGraph(int chanNum,vector<float>& vec,int offset, bool bLog) {
    
    ofSetColor( (bSetThresh && chanNum==curChannel) ? ofColor(245, 58, 135) : ofColor(255) );
    ofRect(0, 0, vec.size(), RECT_HEIGHT);
    
    
    ofSetColor(255);
    vector<float> &thresh = channels[chanNum].thresholds;
    if (bLog) {
        for (int i=0; i<thresh.size(); i++) {
            float y =  20*log(thresh[i])/log(10) ;
            ofSetColor( (bSetThresh && i==curThresh && chanNum==curChannel) ? ofColor(245, 58, 135) : ofColor(255) );
...
void testApp::draw(){
    ...
    for (vector<star>::iterator iter=stars.begin(); iter!=stars.end(); iter++) {
        ...
        if (iter->bTriggered) {
            ofPushMatrix();
            ofRotate(iter->angle, iter->rotationAxis.x, iter->rotationAxis.y, iter->rotationAxis.z);
            color.a = 255*(1.0f-(float)(ofGetElapsedTimeMillis()-iter->triggerTime)/3000.0f);
            ofSetColor(color.r,color.g,color.b,color.a);
            ofCircle(x, y, (ofGetElapsedTimeMillis()-iter->triggerTime)*iter->velocity/1000);
            ofPopMatrix();
        }   
    ...
    if (bDebug) {
        ...
        for (vector<channel>::iterator iter=channels.begin();iter!=channels.end();iter++) {
            ...
            ofPushMatrix();
            ofTranslate(0, 10, 0);
            drawGraph(distance(channels.begin(),iter),iter->peakVec, bufferCounter,true);
            ofPopMatrix();
           
            ofPushMatrix();
            ofTranslate(0, RECT_HEIGHT+20, 0);
            drawGraph(distance(channels.begin(),iter),iter->peakVec, bufferCounter,false);
            ofPopMatrix();
            
            ofPushMatrix();
            ofTranslate(0, 2*RECT_HEIGHT+30, 0);
            drawGraph(distance(channels.begin(),iter),iter->envelopeVec, bufferCounter,false);
            ofPopMatrix();
                       
            ofPushMatrix();
            ofTranslate(0, 3*RECT_HEIGHT+40, 0);
            drawGraph(distance(channels.begin(),iter),iter->limiterVec, bufferCounter,false);
            ofPopMatrix();
    ...
    if (bClouds) {
        ofSetColor(255);
        clouds.getImage().setAnchorPoint(scale*ofGetWidth()/2, scale*ofGetHeight()/2);
        clouds.getImage().draw(ofGetWidth()/2, ofGetHeight()/2, scale*ofGetWidth(), scale*ofGetHeight());
...
void testApp::audioIn(float * input, int bufferSize, int nChannels){	
    ...
    if (bTriggeredOn && bufferCounter-trigBuf>rest) {
        bTriggeredOn = false;
    }
...
//        bool bTriggeredOn = c.bTriggered;
        ...
        for(int i=0;i<bufferSize;i++)
        {
            ...
            if (!bTriggeredOn && !c.bTriggered &&  c.envelope>c.thresholds[THRESHOLD_TRIGGER_ON]) {
                bTriggeredOn = true;
                c.bTriggered = true;
                c.pos=ofVec3f(ofGetWidth()/2,ofGetHeight()/2)+ofVec3f(ofRandom(ofGetHeight()/2),0,0).rotate(ofRandom(360), ofVec3f(0,0,1));
                trigBuf = bufferCounter;
                
                star &s=stars[rand() % stars.size()];
                s.bTriggered = true;
                s.triggerTime = ofGetElapsedTimeMillis();
                s.angle = ofRandom(90);
                s.rotationAxis = ofVec3f(ofRandomf(),ofRandomf(),0);
                s.velocity = ofRandom(50, 100);
...
void testApp::save() {
    ...
    xml.addTag("rest");
    xml.setValue("rest", rest);
    xml.addTag("pondScale");
    xml.setValue("pondScale", pondScale);
...
void testApp::keyPressed(int key){
    ...
    switch (key) {
        ...
        case OF_KEY_UP: 
            if (bThreshDown) {
                for (vector<channel>::iterator iter=channels.begin(); iter!=channels.end(); iter++) {
                    iter->thresholds[curThresh]+=0.0001;
                }  
                save();
            } else if (!bSetThresh) {
                bSetThresh = true;
            } else {
                channels[curChannel].thresholds[curThresh]+=0.0001;
                save();
            }
         break;
        case OF_KEY_DOWN: 
        ...
        case OF_KEY_RIGHT:
            if (!bSetThresh) {
                ...
                curChannel = (curChannel+1) % channels.size();
            }
        break;
        case OF_KEY_LEFT:
            if (!bSetThresh) {
                ...
                curChannel = (curChannel-1+channels.size()) % channels.size();
            }
        break;
        case ' ':
            bSetThresh=!bSetThresh;
            break;
